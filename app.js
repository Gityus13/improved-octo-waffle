    ...
    try {
      ...
      if (currentResponse.ok) {
        ...
        const forecastResponse = await fetch(
          `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${API_KEY}&units=${
            isMetric ? "metric" : "imperial"
          }`
        )
        ...
        if (forecastResponse.ok) {
          ...
          const hourlyProcessed = forecastData.list
            .slice(0, 24)
            .map((item, index) => ({
      ...
      navigator.geolocation.getCurrentPosition(async position => {
        ...
        try {
          const response = await fetch(
            `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${API_KEY}&units=${
              isMetric ? "metric" : "imperial"
            }`
          )
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <Card className="bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-white/20 transition-all duration-300 hover:scale-105">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xl font-bold">
                    {currentWeather.wind.speed} {isMetric ? "m/s" : "mph"}
                  </p>
                </CardContent>
              </Card>
